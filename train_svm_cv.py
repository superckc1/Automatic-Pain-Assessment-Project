...
logging.basicConfig(filename='/cs/home/alykc8/svm_cv_training5.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
for train_index, val_index in kf.split(subject_ids):
    ...
    for i in range(num_batches):
        ...
        svm_model.fit(batch_data, batch_labels)

        batch_train_predictions = svm_model.predict(batch_data)
        all_train_predictions.extend(batch_train_predictions)
        all_train_labels.extend(batch_labels)

    train_loss = mean_squared_error(all_train_labels, all_train_predictions)
